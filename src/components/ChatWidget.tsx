...
function AgentMark({ compact = false }: { compact?: boolean }) {
  return (
    ...
    >
      ...
      <span className="mono relative text-[10px] font-semibold tracking-normal text-primary">
        MC
      </span>
    </span>
  );
        ...
        >
          ...
          <span className="hud-corner relative grid grid-cols-[auto_minmax(0,1fr)] items-center gap-2 rounded-full border border-primary/35 bg-card/90 py-1.5 pl-1.5 pr-3 text-left shadow-2xl backdrop-blur-xl transition-colors group-hover:border-primary/70">
            ...
            <span className="flex min-w-0 flex-col leading-none">
              <span className="mono text-[10px] font-semibold tracking-[0.14em] text-primary">
                MC-AI
              </span>
              <span className="mt-1 text-[11px] font-medium text-foreground/85">
                Ask Jeet's profile
              </span>
            </span>
          </span>
      ...
      {open && (
        ...
        >
          ...
          <p id="mcai-desc" className="sr-only">
            MC-AI is an assistant grounded in Jeet Soni's profile. Type a question and press Enter
            to send. Press Escape to close this panel.
          </p>
          ...
          >
              ...
              >
                ...
                >
                  <span className="sr-only">
                    {m.role === "user" ? "You said:" : "MC-AI replied:"}
                  </span>
            ...
            {loading && (
              <div className="flex gap-2.5 animate-fade-in">
                <div
                  className="grid h-7 w-7 shrink-0 place-items-center rounded-lg border border-primary/30 bg-primary/15 text-primary"
                  aria-hidden="true"
                >
                  <RadioTower className="h-3.5 w-3.5" />
                </div>
          ...
          {messages.length <= 1 && (
            <div className="border-t border-border bg-background/60 px-3.5 pb-2.5 pt-3">
              <div className="mb-2 flex items-center gap-1.5">
                ...
                <span
                  id="mcai-prompts"
                  className="mono text-[9px] font-medium tracking-[0.16em] text-muted-foreground"
                >
                  QUICK PROMPTS
                </span>
              </div>
