...
export function Preloader({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div
      className="preloader fixed inset-0 z-[200] grid place-items-center overflow-hidden"
      style={{
        background:
          "radial-gradient(ellipse at center, oklch(0.16 0.02 262), oklch(0.08 0.015 260) 70%)",
      }}
    >
      ...
      <div className="relative flex flex-col items-center gap-7 px-6">
        ...
        <div className="relative h-56 w-56 grid place-items-center">
          <svg viewBox="0 0 220 220" className="absolute inset-0 -rotate-90">
            ...
            <circle
              ...
              style={{
                filter: "drop-shadow(0 0 10px color-mix(in oklab, var(--cyan) 55%, transparent))",
          ...
          <div className="absolute -bottom-1 mono text-[10px] tracking-[0.45em] text-[var(--cyan)]/80">
            {Math.round(pct)}%
          </div>
        </div>
        ...
        <div className="text-center">
          <div className="text-[15px] font-medium tracking-tight text-foreground/90">
            Jeet Soni · Operating System
          </div>
          <div className="mono text-[10.5px] tracking-[0.35em] text-muted-foreground mt-1">
            MISSION CONTROL · v1
          </div>
        </div>
        ...
        <div className="w-[min(440px,92vw)] glass rounded-xl px-4 py-3 hud-corner">
          <div className="flex items-center gap-2 mono text-[10.5px] text-[var(--cyan)]">
            ...
            <span className="ml-auto opacity-70">
              {Math.min(step, BOOT_LINES.length)}/{BOOT_LINES.length}
            </span>
          </div>
    ...
    <header className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[min(1180px,94vw)]">
      <div className="glass rounded-full px-3 py-2 flex items-center gap-2">
        <Link to="/" className="flex items-center gap-2 px-2">
          <div className="relative h-7 w-7 rounded-md bg-gradient-to-br from-[var(--cyan)] to-[var(--electric)] grid place-items-center text-background font-bold text-[11px]">
            JS
          </div>
          <span className="mono text-[11.5px] tracking-[0.2em] text-foreground/80 hidden sm:inline">
            JEET.SONI
          </span>
        </Link>
        <nav className="ml-auto hidden lg:flex items-center gap-0.5">
          {NAV_ITEMS.map((it) => (
            <Link
              ...
              activeProps={{
                className:
                  "px-3 py-1.5 text-[12px] mono tracking-widest text-[var(--cyan)] bg-[color:var(--cyan)]/10 rounded-full",
              }}
            >
        ...
        <button
          onClick={() => setOpen((o) => !o)}
          className="lg:hidden ml-auto btn-ghost !py-2 !px-2.5"
          aria-label="Menu"
        >
          ...
          <div className="grid grid-cols-2 gap-2">
            {NAV_ITEMS.map((it) => (
              <Link
                ...
                activeProps={{
                  className:
                    "rounded-lg px-3 py-2 mono text-[11.5px] tracking-widest text-[var(--cyan)] bg-[color:var(--cyan)]/10 border border-[color:var(--cyan)]/40",
                }}
              >
                {it.label.toUpperCase()}
              </Link>
    ...
    <footer className="relative py-10 border-t border-white/5 mt-10">
      <div className="mx-auto max-w-7xl px-6 flex flex-wrap items-center justify-between gap-3">
        <div className="mono text-[11px] tracking-widest text-muted-foreground">
          © {new Date().getFullYear()} JEET SONI · ALL SIGNALS RESERVED
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => {
              downloadResume("footer").catch(() => {});
            }}
          ...
          <a href={`mailto:${PROFILE.email}`} className="btn-ghost !px-3" aria-label="Email">
    ...
    <section className="relative pt-32 pb-14 overflow-hidden bg-noise">
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          {code && (
            <span className="mono text-[11px] tracking-widest text-muted-foreground">{code}</span>
          )}
        </div>
        ...
        {subtitle && (
          <p className="mt-4 max-w-2xl text-muted-foreground text-[15.5px] leading-relaxed">
            {subtitle}
          </p>
        )}
        {children}
