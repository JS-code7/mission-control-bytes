...
export const Route = createFileRoute("/lab")({
  ...
  head: () =>
    pageHead({
      ...
      description:
        "Try live demos built by Jeet Soni: a profile-grounded AI concierge, an adaptive traffic simulator, and an ML research console.",
      type: "website",
    }),
});
...
function ChatbotDemo() {
  ...
  const [msgs, setMsgs] = useState<{ from: "user" | "bot"; text: string }[]>([
    {
      from: "bot",
      text: "Concierge online. Ask me about Jeet's stack, projects, or how to collab.",
    },
  ]);
  const send = () => {
    ...
    setTimeout(() => {
      ...
      else if (a.includes("stack") || a.includes("skill"))
        reply = "Core: AI/ML, Cybersecurity, Robotics & Electronics, Web (React/TS), Azure cloud.";
      else if (a.includes("binbuddy"))
        reply = "BinBuddy is an incubated, funded startup — IoT bins + vision + cloud routing.";
    ...
    <div className="glass rounded-2xl p-4 hud-corner h-full flex flex-col">
      ...
      <div className="flex-1 space-y-2 overflow-auto max-h-[260px] pr-1">
        {msgs.map((m, i) => (
          <div
            key={i}
            className={`text-[13px] leading-snug ${m.from === "bot" ? "text-foreground/85" : "text-foreground"}`}
          >
            <span
              className={`mono text-[10px] mr-2 ${m.from === "bot" ? "text-[var(--cyan)]" : "text-[var(--purple-glow)]"}`}
            >
              {m.from === "bot" ? "BOT" : "YOU"}
            </span>
            {m.text}
...
function TrafficDemo() {
  ...
  const light = (dir: "N" | "E" | "S" | "W") =>
    phase === dir ? "var(--cyan)" : "rgba(255,255,255,0.15)";
  return (
    <div className="glass rounded-2xl p-4 hud-corner h-full">
      <div className="flex items-center gap-2 mono text-[11px] text-[var(--cyan)]">
        ...
        <button
          ...
          className="ml-auto btn-ghost !py-1 !px-2 text-[11px]"
        >
      ...
      <div className="relative mx-auto aspect-square w-[240px]">
        ...
        <div
          className="absolute left-1/2 top-2 -translate-x-1/2 h-3 w-3 rounded-full"
          style={{ background: light("N"), boxShadow: phase === "N" ? "0 0 18px var(--cyan)" : "" }}
        />
        <div
          className="absolute right-2 top-1/2 -translate-y-1/2 h-3 w-3 rounded-full"
          style={{ background: light("E"), boxShadow: phase === "E" ? "0 0 18px var(--cyan)" : "" }}
        />
        <div
          className="absolute left-1/2 bottom-2 -translate-x-1/2 h-3 w-3 rounded-full"
          style={{ background: light("S"), boxShadow: phase === "S" ? "0 0 18px var(--cyan)" : "" }}
        />
        <div
          className="absolute left-2 top-1/2 -translate-y-1/2 h-3 w-3 rounded-full"
          style={{ background: light("W"), boxShadow: phase === "W" ? "0 0 18px var(--cyan)" : "" }}
        />
        <div
          className="absolute left-1/2 -translate-x-1/2 h-2 w-3 rounded-sm bg-[var(--gold)] transition-all"
          style={{ top: phase === "S" ? "70%" : "20%" }}
        />
      </div>
      <div className="mono text-[11px] text-muted-foreground mt-2 text-center">
        PHASE · {phase} · adaptive timing engaged
      </div>
    </div>
  );
}
    ...
    <div className="glass rounded-2xl p-4 hud-corner h-full flex flex-col">
      ...
      <div className="grid grid-cols-1 gap-2">
        {items.map((it) => (
          <button
            key={it.label}
            onClick={() => setLog((l) => [...l, "» " + it.t].slice(-6))}
            className="btn-ghost justify-between"
          >
            <span className="flex items-center gap-2">
              <Zap className="h-3.5 w-3.5 text-[var(--cyan)]" /> {it.label}
            </span>
      ...
      <div className="mt-3 bg-black/30 rounded-md border border-white/10 p-3 flex-1 min-h-[110px] text-[13px] animate-fade-in">
        ...
        {!err && !result && !loading && (
          <div className="mono text-[11px] text-muted-foreground">
            » awaiting input · powered by Lovable AI
          </div>
        )}
        {loading && (
          <div className="mono text-[11px] text-muted-foreground">
            » computing · streaming inference
          </div>
        )}
        ...
        return (
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="ml-auto mono text-[10px] opacity-70">
        ...
        return (
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-2 py-1 rounded-md bg-[var(--purple-glow)]/15 border border-[var(--purple-glow)]/30">
              <span className="mono text-[10px] tracking-widest text-[var(--purple-glow)]">
                {topic.toUpperCase()}
              </span>
            </div>
  ...
  return (
    <>
      <PageHero
        ...
        eyebrow={
          <>
            <FlaskConical className="h-3 w-3" /> SYSTEM · 03 · LAB
          </>
        }
        code="LAB · LIVE"
        title={
          <>
            Interactive <span className="text-gradient">Lab</span>
          </>
        }
      ...
      <section className="relative pb-6">
        <div className="mx-auto max-w-7xl px-6 mb-4 flex items-end justify-between">
          <div>
            <div className="mono text-[10.5px] tracking-[0.22em] text-[var(--cyan)]">
              ML · MODELS · ONLINE
            </div>
            <h2 className="text-2xl md:text-3xl font-semibold mt-1">
              Real inference, in your browser
            </h2>
            <p className="text-muted-foreground text-[14px] mt-1 max-w-2xl">
              Three live ML demos powered by the Lovable AI Gateway. Type anything — the model
      ...
      <section className="relative pb-20 pt-10">
        <div className="mx-auto max-w-7xl px-6 grid md:grid-cols-3 gap-5">
          {[
            {
              t: "Edge perception",
              d: "On-device computer vision for fill-level and contamination classification.",
            },
            {
              t: "Policy-aware LLMs",
              d: "Guardrails, refusal logic, and tool routing for high-stakes assistants.",
            },
            {
              t: "Hardware-in-the-loop",
              d: "Sim-to-real bridges so models meet sensors before they meet customers.",
            },
          ].map((p) => (
        ...
        <div className="mx-auto max-w-7xl px-6 mt-10">
          <Link to="/brain" className="btn-hero">
            Open Brain Map <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      </section>
    </>
