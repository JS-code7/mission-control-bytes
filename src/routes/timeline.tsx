...
export const Route = createFileRoute("/timeline")({
  ...
  head: () =>
    pageHead({
      ...
      description:
        "Career checkpoints for Jeet Soni: beginner, builder, operator, engineer, founder — with the milestone behind each leap.",
      type: "website",
    }),
});
...
const STAGES = [
  {
    name: "Beginner",
    color: "var(--cyan)",
    note: "Curiosity-led tinkering. Code and circuits as toys.",
  },
  {
    name: "Intermediate",
    color: "var(--teal)",
    note: "First shipped things. Real users, real bugs, real learning.",
  },
  {
    name: "Builder",
    color: "var(--electric)",
    note: "End-to-end ownership. Pixels to PCBs to deploys.",
  },
  {
    name: "Innovator",
    color: "var(--purple-glow)",
    note: "New systems, funded startup, building for the field.",
  },
];
  ...
  return (
    <>
      <PageHero
        ...
        eyebrow={
          <>
            <Star className="h-3 w-3" /> SYSTEM · 05 · TIMELINE
          </>
        }
        code={`${TIMELINE.length} CHECKPOINTS · ACTIVE`}
        title={
          <>
            Evolution <span className="text-gradient">Timeline</span>
          </>
        }
                ...
                <div key={t.year} className="relative">
                  ...
                  <div className="glass mt-6 rounded-xl p-4 hud-corner card-hover">
                    <div className="mono text-[10.5px] tracking-widest text-[var(--cyan)]">
                      {t.year} · {t.label.toUpperCase()}
                    </div>
                    <div className="mt-2 text-[14px] text-foreground/90">{t.text}</div>
                  </div>
                  {i < TIMELINE.length - 1 && (
                    <div className="hidden md:block absolute top-9 right-[-12px] w-3 h-px bg-[var(--cyan)]/40" />
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="mt-6 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {STAGES.map((s, i) => (
              <div key={s.name} className="reveal glass rounded-2xl p-5 hud-corner card-hover">
                <div className="mono text-[10.5px] tracking-widest" style={{ color: s.color }}>
                  STAGE · 0{i + 1}
                </div>
          ...
          <div className="mt-10">
            <Link to="/modules" className="btn-hero">
              Browse Active Modules <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
        </div>
      </section>
