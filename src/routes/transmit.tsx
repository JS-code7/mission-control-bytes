...
export const Route = createFileRoute("/transmit")({
  ...
  head: () =>
    pageHead({
      ...
      description:
        "Live system messages, activity logs, and uplink status from Jeet Soni's mission control.",
      type: "website",
    }),
});
  ...
  return (
    <>
      <PageHero
        ...
        eyebrow={
          <>
            <Terminal className="h-3 w-3" /> SYSTEM · 07 · TRANSMIT
          </>
        }
        code="CHANNEL · OPEN"
        title={
          <>
            Transmission <span className="text-gradient">Console</span>
          </>
        }
        ...
        <div className="mx-auto max-w-7xl px-6 grid lg:grid-cols-[1.4fr_1fr] gap-5">
          ...
          <div className="reveal glass rounded-2xl p-5 hud-corner scanline">
            <div className="flex items-center gap-2 mono text-[11px] text-[var(--cyan)]">
              <Terminal className="h-3.5 w-3.5" /> /transmit/console{" "}
              <span className="ml-auto opacity-70">LIVE</span>
            </div>
            ...
            <div className="mono text-[12.5px] leading-relaxed bg-black/40 border border-white/10 rounded-lg p-4 min-h-[320px] max-h-[420px] overflow-auto">
              {log.map((l, i) => (
                <div key={i} className="text-foreground/85">
                  <span className="text-[var(--cyan)]">{l.replace("›", "›")}</span>
                </div>
              ))}
          ...
          <div className="grid grid-cols-1 gap-4">
            {[
              ...
              <div
                key={s.l}
                className="reveal glass rounded-2xl p-4 hud-corner card-hover flex items-center gap-4"
              >
                <div
                  className="grid place-items-center h-10 w-10 rounded-lg"
                  style={{
                    background: `color-mix(in oklab, ${s.c} 18%, transparent)`,
                    border: `1px solid color-mix(in oklab, ${s.c} 50%, transparent)`,
                  }}
                >
          ...
          <div className="reveal glass rounded-2xl p-6 hud-corner flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <div className="mt-1 text-lg">
                Operator standing by at{" "}
                <span className="text-foreground font-semibold">{PROFILE.email}</span>
              </div>
            </div>
            <Link to="/contact" className="btn-hero">
              Open Contact Channel <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="flex items-end justify-between gap-4 mb-5">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              One tap to draft an email, or copy the full template to send anywhere.
            </p>
          </div>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {TEMPLATES.map((t) => (
            <div
              key={t.tag}
              className="reveal glass rounded-2xl p-4 hud-corner card-hover flex flex-col gap-3"
            >
              <div className="flex items-center gap-2">
                ...
                <span className="mono text-[9.5px] tracking-widest px-2 py-0.5 rounded-full border border-white/10 text-muted-foreground">
                  {t.tag}
                </span>
              </div>
              <div>
              ...
              <div className="mt-auto flex items-center gap-2">
                <button
                  onClick={() => open(t)}
                  className="flex-1 inline-flex items-center justify-center gap-1.5 text-[12px] px-3 py-2 rounded-md bg-gradient-to-br from-[var(--cyan)] to-[var(--electric)] text-background font-medium hover:opacity-90 transition"
                >
                ...
                <button
                  ...
                  className="grid place-items-center h-9 w-9 rounded-md border border-white/10 bg-white/[0.04] hover:bg-white/[0.08] text-foreground/80 transition"
                >
                  {copied === t.tag ? (
